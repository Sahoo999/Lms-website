 ...
 const fetchDashboarddData = async () =>{
   try {
     ...
     const {data} = await axios.get(backendUrl + '/api/educator/dashboard', {headers: {Authorization: `Bearer ${token}` }})
      ...
      if(data.success){
        setDashboarddData(data.dashboarddData);
      }else{
  ...
  return dashboarddData ? (
    ...
    <div className='flex flex-wrap gap-5 items-center'>
      <div className='flex items-center gap-3 shadow-card border border-blue-500 p-4 w-56 rounded-md'>
        ...
        <div>
          <p className='text-2xl font-medium text-gray-600 '>{dashboarddData.enrolledStudentsData.length} </p>
      ...
      <div className='flex items-center gap-3 shadow-card border border-blue-500 p-4 w-56 rounded-md'>
        ...
        <div>
          <p className='text-2xl font-medium text-gray-600 '>{dashboarddData.totalCourses} </p>
      ...
      <div className='flex items-center gap-3 shadow-card border border-blue-500 p-4 w-56 rounded-md'>
        ...
        <div>
          <p className='text-2xl font-medium text-gray-600 '>{currency} {dashboarddData.totalEarnings} </p>
      ...
      <div>
        ...
        <tbody className='text-sm text-gray-500'>
          {dashboarddData.enrolledStudentsData.map((item, index) =>(
